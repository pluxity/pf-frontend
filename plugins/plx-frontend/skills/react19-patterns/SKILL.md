...
allowed-tools: Read, Glob, Grep, mcp__plugin_context7_context7__resolve-library-id, mcp__plugin_context7_context7__query-docs
---
...
React API에 대해 확신이 없을 때, Context7 MCP로 최신 공식 문서를 확인합니다:

1. `resolve-library-id`로 React 라이브러리 ID 조회
2. `query-docs`로 해당 API 최신 문서 fetch

```
예: "use() hook의 정확한 사용법은?"
→ Context7에서 React 공식 문서의 use() 레퍼런스를 가져와서 답변
```

**Context7으로 확인하면 좋은 항목:**

- `use()`, `useOptimistic`, `useActionState` 등 React 19 신규 Hook API
- Suspense 경계 패턴 변경사항
- React Compiler 호환성 가이드
