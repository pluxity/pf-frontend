    ...
    >
      {steps.map((step, index) => {
        ...
        return (
          <div
            key={index}
            className={cn("flex", isHorizontal ? "flex-1 flex-col items-center" : "flex-row gap-4")}
          >
            <div className={cn("flex items-center", isHorizontal ? "w-full" : "flex-col")}>
              ...
              >
                {isCompleted ? <Check size="md" /> : index + 1}
              </div>
            ...
            <div className={cn(isHorizontal ? "mt-3 text-center" : "flex-1 pb-8 last:pb-0")}>
              ...
              {step.description && (
                <p className="mt-1 text-xs text-[#808088]">{step.description}</p>
              )}
            </div>
          </div>
        );
