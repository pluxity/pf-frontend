...
function FileItem({
  ...
  ...props
    ...
    const isUploading = file.status === "uploading" || (file.progress !== undefined && file.progress < 100);
    ...
    return (
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium text-[#333340]">
            {file.name}
          </p>
          <div className="flex items-center gap-2">
            <span
              className={cn(
                "text-xs",
                isError ? "text-error-brand" : "text-[#808088]"
              )}
            >
        ...
        {isComplete && (
          <CheckCircle size="md" className="flex-shrink-0 text-success-brand" />
        )}
        {isError && (
          <AlertCircle size="md" className="flex-shrink-0 text-error-brand" />
        )}
